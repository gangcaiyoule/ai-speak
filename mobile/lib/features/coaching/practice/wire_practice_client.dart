...
final class WirePracticeClient implements PracticeClient {
  ...
  Future<_Response> _request(String method, String path, {Map<String, Object?>? body}) async {
    ...
    if (token == null || token.isEmpty) throw const PracticeClientException('authentication required', 401);
    try {
      ...
      return _Response(response.statusCode, await response.stream.bytesToString());
    } on Exception catch (error) {
      // 网络层异常（原生平台为 SocketException/IOException，web 平台为
      // http.ClientException），统一归为练习服务不可用。
