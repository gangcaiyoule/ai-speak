...
final class WirePracticePlanClient implements PracticePlanClient {
  ...
  Future<_Response> _request(String method, String path, {Map<String, Object?>? body}) async { final token = await _tokenProvider(); if (token == null || token.isEmpty) throw const PracticePlanClientException('authentication required', 401); final request = http.Request(method, _baseUri.resolve(path))..headers['authorization'] = 'Bearer $token'..headers['content-type'] = 'application/json'; if (body != null) request.body = jsonEncode(body); final response = await _http.send(request); return _Response(response.statusCode, await response.stream.bytesToString()); }
