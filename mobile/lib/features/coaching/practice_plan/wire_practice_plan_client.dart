...
final class WirePracticePlanClient implements PracticePlanClient {
  ...
  @override Future<List<PracticePlan>> listPlans() async { final response = await _request('GET', '/v1/practice-plans'); if (response.statusCode != 200) throw _error(response); final root = jsonDecode(response.body); if (root is! Map || root['plans'] is! List) throw const PracticePlanClientException('invalid practice plan response'); return (root['plans'] as List).map(_decode).toList(growable: false); }
  @override Future<PracticePlan> getPlan(String id) async { final response = await _request('GET', '/v1/practice-plans/${Uri.encodeComponent(id)}'); if (response.statusCode != 200) throw _error(response); final root = jsonDecode(response.body); return _decode(root is Map ? root['plan'] : null); }
  @override Future<PracticePlan> archivePlan(String id) async { final response = await _request('POST', '/v1/practice-plans/${Uri.encodeComponent(id)}/archive'); if (response.statusCode != 200) throw _error(response); final root = jsonDecode(response.body); return _decode(root is Map ? root['plan'] : null); }
  Future<_Response> _request(String method, String path, {Map<String, Object?>? body}) async { final token = await _tokenProvider(); if (token == null || token.isEmpty) throw const PracticePlanClientException('authentication required', 401); final request = http.Request(method, _baseUri.resolve(path))..headers['authorization'] = 'Bearer $token'..headers['content-type'] = 'application/json'; if (body != null) request.write(jsonEncode(body)); final response = await _http.send(request); return _Response(response.statusCode, await response.stream.bytesToString()); }
