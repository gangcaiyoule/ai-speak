...
func (r Runner) Run(ctx context.Context, db Executor) error {
	if db == nil {
		return fmt.Errorf("migration executor is nil")
	}
	...
	for _, m := range migrations {
		if m.Version < 1 || m.Name == "" || m.SQL == "" {
			return fmt.Errorf("invalid migration %d", m.Version)
		}
		if applied[m.Version] {
			continue
		}
		if err := db.ExecContext(ctx, m.SQL); err != nil {
			return fmt.Errorf("apply migration %d (%s): %w", m.Version, m.Name, err)
		}
		if err := db.ExecContext(ctx, "INSERT INTO schema_migrations(version, name) VALUES ($1, $2)", m.Version, m.Name); err != nil {
