...
const (
	ScoreabilityProvisional  Scoreability = "PROVISIONAL"
	ScoreabilityInsufficient Scoreability = "INSUFFICIENT"
)

type ScoreScale string

const (
	ScoreScalePercentage100 ScoreScale = "PERCENTAGE_100"
	ScoreScaleIELTSBand     ScoreScale = "IELTS_BAND_9"
)

type FeedbackCategory string

const (
	FeedbackCorrection            FeedbackCategory = "CORRECTION"
	FeedbackStrength              FeedbackCategory = "STRENGTH"
	FeedbackRecommendedExpression FeedbackCategory = "RECOMMENDED_EXPRESSION"
)

type RepracticeMode string

const (
	RepracticeNone         RepracticeMode = "NONE"
	RepracticeSameQuestion RepracticeMode = "SAME_QUESTION"
)
...
type Report struct {
	...
	Result      *FormalReport  `json:"result,omitempty"`
...
type FormalReport struct {
	...
	ScoreabilityStatus Scoreability     `json:"scoreability_status"`
	Summary            string           `json:"summary"`
...
type Dimension struct {
	...
	Examples     []Finding  `json:"recommended_examples"`
}
type Finding struct {
...
type FeedbackItem struct {
	...
	RepracticeMode RepracticeMode   `json:"repractice_mode"`
	CreatedAt      time.Time        `json:"created_at"`
}
...
func (value FormalReport) Valid() bool {
	...
	for _, question := range value.Questions {
		if !validUUID(question.ID) || question.Position < 1 || !validText(question.Text, 16384) || questions[question.ID] || (question.ParentQuestionID != "" && question.ParentQuestionID == question.ID) {
			return false
		}
		questions[question.ID] = true
		if question.Answer != nil {
			if !validUUID(question.Answer.TurnID) || !validText(question.Answer.Transcript, 65536) || turns[question.Answer.TurnID] {
				return false
			}
			turns[question.Answer.TurnID] = true
		}
	}
	for _, question := range value.Questions {
		if question.ParentQuestionID != "" && !questions[question.ParentQuestionID] {
			return false
		}
	}
	for _, dimension := range value.Dimensions {
		if !dimension.valid(value.ScoreabilityStatus) || dimensions[dimension.Key] {
			return false
		}
		dimensions[dimension.Key] = true
		for _, group := range [][]Finding{dimension.Strengths, dimension.Improvements, dimension.Examples} {
			for _, finding := range group {
				if !finding.valid(turns) || findings[finding.ID] {
					return false
				}
	...
	for _, action := range value.PriorityActions {
		...
		if improvements[action.FindingID] != action.DimensionKey || actions[key] {
			return false
		}
		actions[key] = true
	}
...
func (item FeedbackItem) Valid() bool {
	if !validUUID(item.ID) || !validUUID(item.EvaluationID) || item.Position < 1 || !item.Evidence.valid(nil) || !validText(item.Recommendation, 4096) || item.CreatedAt.IsZero() {
		return false
	}
	if item.Category == FeedbackStrength {
		return item.RepracticeMode == RepracticeNone && item.Correction == ""
	}
...
func (value Dimension) valid(scoreability Scoreability) bool {
	...
	if value.Score != nil && (math.IsNaN(*value.Score) || math.IsInf(*value.Score, 0) || *value.Score < 0 || (value.Scale == ScoreScalePercentage100 && *value.Score > 100) || (value.Scale == ScoreScaleIELTSBand && (*value.Score > 9 || math.Mod(*value.Score*2, 1) != 0))) {
		return false
	}
...
func (value Finding) valid(turns map[string]bool) bool {
	if !validIdentifier(value.ID) || !validText(value.Message, 2048) || (value.Suggestion != "" && !validText(value.Suggestion, 2048)) || value.Evidence == nil || len(value.Evidence) > 8 {
		return false
	}
	for _, evidence := range value.Evidence {
		if !evidence.valid(turns) {
			return false
		}
	}
	return true
}
func (value Evidence) valid(turns map[string]bool) bool {
	return validUUID(value.EvidenceRefID) && validUUID(value.TurnID) && value.EvidenceRefID == value.TurnID && (turns == nil || turns[value.TurnID]) && value.StartUTF8Byte >= 0 && value.EndUTF8Byte > value.StartUTF8Byte && value.EndUTF8Byte-value.StartUTF8Byte == len(value.OriginalExcerpt) && validText(value.OriginalExcerpt, 16384)
}
func validUUID(value string) bool       { return uuidPattern.MatchString(value) }
func validIdentifier(value string) bool { return identifierPattern.MatchString(value) }
func validText(value string, maximum int) bool {
	return utf8.ValidString(value) && value == strings.TrimSpace(value) && value != "" && len(value) <= maximum && !strings.ContainsRune(value, '\x00')
}
func ratio(value float64) bool {
	return !math.IsNaN(value) && !math.IsInf(value, 0) && value >= 0 && value <= 1
}
func validScene(value SceneType) bool {
	return value == SceneIELTSSpeaking || value == SceneInterview || value == SceneOverseasDaily || value == SceneOverseasWorkplace
}
func validScoreability(value Scoreability) bool {
	return value == ScoreabilityProvisional || value == ScoreabilityInsufficient
}

type Repository interface {
	Create(context.Context, Report) (Report, error)
	FindByID(context.Context, string, string) (Report, error)
	FindBySession(context.Context, string, string) (Report, error)
	List(context.Context, HistoryFilter) (HistoryPage, error)
}
